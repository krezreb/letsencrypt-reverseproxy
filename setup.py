...
TEMPLATE_FILE_444 = os.environ.get(
    "TEMPLATE_FILE_444", "/etc/nginx/conf.d/nginx_default444.conf.tpl"
)
TEMPLATE_FILE_HTTP = os.environ.get(
    "TEMPLATE_FILE_HTTP", "/etc/nginx/conf.d/nginx_http.conf.tpl"
)
TEMPLATE_FILE_HTTPS = os.environ.get(
    "TEMPLATE_FILE_HTTPS", "/etc/nginx/conf.d/nginx_https.conf.tpl"
)
...
def run(cmd, splitlines=False, env=os.environ.copy()):
    ...
    proc = Popen(
        cmd, stdout=PIPE, stderr=PIPE, universal_newlines=True, shell=True, env=env
    )
...
def apply_template(template_path, invars, basic_auth_file=None):
    ...
    if basic_auth_file != None:
        ...
        vars["AUTH_BASIC_USER_FILE"] = 'auth_basic_user_file "{}";'.format(
            basic_auth_file
        )
    else:
        vars["AUTH_BASIC"] = ""
    ...
    if CONF_YML != None and os.path.exists(CONF_YML):
        ...
        for k, v in conf["conf"].items():
            ...
            if "PROXY_PASS_TARGET" not in v:
                vars["PROXY_PASS"] = ""
                log("NOTICE: no PROXY_PASS_TARGET provided for {}".format(k))
                continue

            vars["PROXY_PASS"] = "proxy_pass {};".format(v["PROXY_PASS_TARGET"])
            ...
            applied_template = apply_template(
                TEMPLATE_FILE_HTTPS, vars, basic_auth_file
            )
